...
contract PaperReviewHistory {
    ...
    function getPaperReviewHistory(bytes32 paperHash) view public returns (address, string memory, bytes32, bytes32, bytes32[] memory, uint256[] memory, address[] memory, bytes32[] memory, uint256[] memory, int[] memory, int[] memory ){//, string memory, bytes32, bytes32) {//, bytes32, uint256, address, string memory, bytes32, uint256, int8, int8) {
        bytes32 rootHash = getRootPaperHash(paperHash);
        return (papers[rootHash].paperAuthorAddress, papers[rootHash].authorName, papers[rootHash].paperHash, papers[rootHash].parentPaperHash, papers[rootHash].paperHashHistory, papers[rootHash].paperTimestamps, papers[rootHash].reviewerAddresses, papers[rootHash].reviewHashes, papers[rootHash].reviewerTimestamps, papers[rootHash].overallEvaluation, papers[rootHash].reviewerConfidence);//, papers[rootHash].parentPaperHash);
    }
    ...
    function setPaper(address _paperAuthorAddress, string memory _authorName, bytes32 _paperHash) public payable{ // "0xFFcf8FDEE72ac11b5c542428B35EEF5769C409f0", "Jon Doe", "0xca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb"
        require(msg.value >= (1 * (10**18)));
        require(address(msg.sender).balance > 1000000000000000000);
    ...
    function releaseEtherToReviewers(bytes32 _paperHash) payable public {

        address payable[] memory reviewerAddresses = new address payable[](papers[_paperHash].reviewerAddresses.length);
        uint256 reviewerShare = 10000000000000000000 / reviewerAddresses.length;
        for (uint i=0; i<papers[_paperHash].reviewerAddresses.length; i++) {
            reviewerAddresses[i] = address(uint160(papers[_paperHash].reviewerAddresses[i]));
        }
        for (uint i=0; i<reviewerAddresses.length; i++) {
            reviewerAddresses[i].transfer(reviewerShare);
        }
    }

     function updatePaper(bytes32 _oldPaperHash, bytes32 _newPaperHash) public {
        if(_oldPaperHash != _newPaperHash){
    ...
    function setReview(bytes32 _paperHash, address _reviewerAddress, bytes32 _reviewHash, int _overallEvaluation, int _reviewerConfidence) public {
